...
## calculate average score on each test over the years
yearly_avg = {"borough" : [], "year" : [], "regents_exam" : [], "total_tested" : [], "mean_score": [], "percent_65_or_above" : []}

for boro in boroughs:

    boro_subset = cleaned_df[cleaned_df["borough"] == boro]

    boro_subset = boro_subset[["year", "regents_exam", "total_tested", "mean_score", "percent_65_or_above"]]

    for year in sorted(boro_subset["year"].unique()):

        for exam in sorted(boro_subset["regents_exam"].unique()):

            mask = (boro_subset["year"] == year) & (boro_subset["regents_exam"] == exam)

            target_subset = boro_subset[mask]

            yearly_avg["borough"].append(boro)
